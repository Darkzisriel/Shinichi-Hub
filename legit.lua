...
discordLinkButton.Size = UDim2.new(0.5, 0, 0.12, 0) -- nút chiếm nửa ngang
...
local function playDodge()
	...
	char:PivotTo(hrp.CFrame * CFrame.new(offset))
			...
			for _, killer in ipairs(workspace.Players.Killers:GetChildren()) do
				...
				if char and char:FindFirstChild("HumanoidRootPart") and killer:FindFirstChild("HumanoidRootPart") then
					...
					if dist <= 50 then
						...
						if safestSpot then
							local safePos = safestSpot.cframe * CFrame.new(safestSpot.cframe.LookVector * 3)
							...
							if furthestGen then
								char.HumanoidRootPart.CFrame = furthestGen:GetPivot() * CFrame.new(furthestGen:GetPivot().LookVector*3)
...
local function DoAllGenerators()
	...
	while not areAllGeneratorsCompleted() do
		...
		for _, g in ipairs(availableGens) do
			...
			if Players.LocalPlayer.Character and Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
				-- Sửa lỗi: cộng CFrame + Vector3 -> CFrame * CFrame.new(Vector3)
				Players.LocalPlayer.Character.HumanoidRootPart.CFrame = g:GetPivot() * CFrame.new(g:GetPivot().LookVector * 3)
				VisualizePivot(g)
				task.wait(0.5)
