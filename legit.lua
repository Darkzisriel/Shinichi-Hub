...
container.Size = UDim2.new(0.9, 0, 0.5, 0) -- chiếm 90% chiều ngang, 50% chiều dọc
...
logo.Size = UDim2.new(0.25, 0, 0.4, 0) -- logo chiếm 25% ngang, 40% cao
...
discordLinkButton.Size = UDim2.new(0.61, 0, 0.12, 0) -- nút chiếm nửa ngang
...
local function GetFurthestSafePosition(killerPosition)
    ...
    for _, genData in pairs(knownGeneratorPositions) do
        local dist = (genData.position - killerPosition).Magnitude
                ...
                for _, killer in ipairs(workspace.Players.Killers:GetChildren()) do
                    ...
                    if char and char:FindFirstChild("HumanoidRootPart") and killer:FindFirstChild("HumanoidRootPart") then
                        ...
                        if dist <= 50 then
                            ...
                            if safestSpot then
                                local safeCFrame = CFrame.new(safestSpot.position + safestSpot.cframe.LookVector * 3)
                                char.HumanoidRootPart.CFrame = safeCFrame
                                MakeNotif("Smart Escape!", string.format("Escaped to position %d studs from killer!", math.floor((safestSpot.position - killerPos).Magnitude)), 5, Color3.fromRGB(0, 255, 255))
                            else
                                local furthestGen = GetFurthestGenerator()
                                if furthestGen then
                                    local genCFrame = furthestGen:GetPivot() * CFrame.new(0, 0, -3)
                                    char.HumanoidRootPart.CFrame = genCFrame
...
local function DoAllGenerators()
    ...
    while not areAllGeneratorsCompleted() do
        ...
        for _, g in ipairs(availableGens) do
            if g.Progress.Value >= 100 then continue end
            local char = lp.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                char.HumanoidRootPart.CFrame = g:GetPivot() * CFrame.new(0, 0, -3)
                VisualizePivot(g)
                task.wait(0.5)
    ...
    SendWebhook("Generator Autofarm", "Finished all generators, Current Balance: " .. lp.PlayerData.Stats.Currency.Money.Value, 0x00FF00, ProfilePicture, "Crystal Hub 😏")
    task.wait(1)
    teleportToRandomServer()
end
...
local function FlyUp(character)
    local hrp = character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    local startCFrame = hrp.CFrame
    local goalCFrame = startCFrame * CFrame.new(0, 20, 0) 
...
local function StartSoundAnimTracking()
    workspace.DescendantAdded:Connect(function(desc)
        if desc:IsA("Sound") and TargetIds[desc.SoundId] then
    ...
    workspace.DescendantAdded:Connect(function(desc)
        if desc:IsA("Animation") and TargetIds[desc.AnimationId] then
            local char = Players.LocalPlayer.Character or Players.LocalPlayer.CharacterAdded:Wait()
            FlyUp(char)
        end
    end)
...
local function FlyUp(character)
    ...
    local startCFrame = hrp.CFrame
    local goalCFrame = startCFrame * CFrame.new(0, 20, 0) -- bay lên 20 stud
    ...
    task.spawn(function()
        ...
        while totalTime < 6 do
            ...
            tween.Completed:Wait()
            task.wait(1)
            totalTime = totalTime + 3 -- 2s bay + 1s nghỉ
        end
    end)
end

-- Khi vào game mới bắt đầu track Sound/Animation
local function StartSoundAnimTracking()
    workspace.DescendantAdded:Connect(function(desc)
        if desc:IsA("Sound") and TargetIds[desc.SoundId] then
            local char = Players.LocalPlayer.Character or Players.LocalPlayer.CharacterAdded:Wait()
