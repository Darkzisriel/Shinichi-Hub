    ...
    for _, g in ipairs(findGenerators()) do
        ...
        for attempt = 1, 3 do
            ...
            if killer and (Players.LocalPlayer.Character:GetPivot().Position - killer.Position).Magnitude <= 80 then
                local move = (Players.LocalPlayer.Character:GetPivot().Position - killer.Position).Unit * 30
                Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(Players.LocalPlayer.Character:GetPivot().Position + move)
                task.wait(0.5)
            end
