	...
	if map then
		for _, g in ipairs(map:GetChildren()) do
			if g.Name == "Generator" then
				-- Save generator position to our storage
				...
				-- Only add to active list if not completed and no players nearby
...
-- Get furthest safe position from killer
local function GetFurthestSafePosition(killerPosition)
	...
	-- Check all known generator positions
...
-- Enhanced killer avoidance with stored positions
...
-- Enhanced generator automation with position saving
local function DoAllGenerators()
	-- First scan to save all generator positions
	MakeNotif("Mapping", "Scanning and saving all generator positions...", 3, Color3.fromRGB(100, 150, 255))
	findGenerators() -- This will populate knownGeneratorPositions
